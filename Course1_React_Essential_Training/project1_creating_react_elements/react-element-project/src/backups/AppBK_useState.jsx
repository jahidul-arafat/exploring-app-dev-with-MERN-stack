...
// import all the state management hooks i.e. useState, useReducer etc
import {useReducer, useState,} from "react"; // use {} for named imports in JavaScript i.e. functions, objects, values that are exported by name from a module
import './App.css'
...
function RestaurantStatus({openStatus, onStatus}) {
    ...
    }; // shorthand way of writing function in JS
    return (
        // onClick={toggleStatus} --> means, when the button clicked, call the function 'toggleStatus'
        // onClick={toggleStatus()} --> means, run toggleStatus right now and give its result to onClick
        <div>
...
function AppBKUseState() {
    ...
    // Alternative Approach: instead of 'useState', try 'useReducer'
    // useReducer return [stateVariable, aDispatcher_that_dispatch_an_event]
    // const [status, dispatch] = useReducer(tru);
    // // console.log("useReducer: ", what)
    ...
    return (
        // The pattern of using onClick={() => ... } is common when you want to run some inline logic
        // on a user action without defining a separate named function.
        <>
